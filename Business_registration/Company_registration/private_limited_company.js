/* Private Limited Company page functionality */

(function () {
    "use strict";

    const items = document.querySelectorAll(".pvt-reveal");
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    if (reduced) {
        items.forEach(item => item.classList.add("pvt-visible"));
    } else {
        const observer = new IntersectionObserver((entries, obs) => {
            entries.forEach(entry => {
                if (!entry.isIntersecting) return;
                entry.target.classList.add("pvt-visible");
    ...
    const form = document.getElementById("pvtLeadForm");
    const success = document.getElementById("pvtFormSuccess");
    ...
    document.querySelectorAll('a[href^="#"]').forEach(link => {
        link.addEventListener("click", function (event) {
            ...
            if (id === "#pvtLeadForm") {
                ...
                document.getElementById("pvtName")?.focus();
                return;
            }
...
(function () {
    const sideNav = document.getElementById("siSideNav") || document.getElementById("pvtSideNav");
    if (!sideNav) return;
